...
import { generateFromTemplate, getCurrentFilePath, C } from '../helpers.mjs';
...
export function generate([folderFromCli, filenameFromCli, ..._], args) {
    ...
    if (!Boolean(folder)) {
        console.log(`${C.cR("Error")}: tinchi generate [path/to/folder]`);
        console.log(`\t folder param is necessary, either specify it or init a new config with ${C.b("'tinchi init'")}`);
        process.exit(1);
    }
    const results = [];
